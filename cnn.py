# -*- coding: utf-8 -*-
"""
Created on Mon Nov 30 16:37:12 2015
...
def fit_predict(data, labels, action, filename, test_datasets = [], learning_rate=0.3, n_epochs=3, nkerns=[20, 50], batch_size=150, seed=2400):
...
#                          delimiter=',', skiprows=1)
    ...
    data, test_data, labels, test_label = train_test_split(data, labels, test_size=0.3)
...
#               header='ImageId,Label', comments='')
    print "... classification accuracy on 70/30 split:", metrics.accuracy_score(test_label, preds)
    t1 = time.time()
    print("... The run time is: %.03f seconds" %(t1-t0))
